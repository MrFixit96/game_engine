...
func run() error {
    ...
    // and call it something along the lines of "game tracker"
    connectionString := "postgres://postgres:postgres@localhost/game_engine?sslmode=disable"
    ...
    // create game service
    gameService := api.NewGameService(storage)

    server := app.NewServer(router, userService, gameService)
